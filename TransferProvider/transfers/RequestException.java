...
class RequestException extends Exception {
    ...
    public RequestException(int finalStatus, String message, Throwable t) {
        super(message, t);
        mFinalStatus = finalStatus;
    }
    ...
    public static RequestException throwUnhandledHttpError(int code, String message)
            throws RequestException {
        final String error = "Unhandled HTTP response: " + code + " " + message;
        if (code >= 400 && code < 600) {
            throw new RequestException(code, error);
        } else if (code >= 300 && code < 400) {
            throw new RequestException(STATUS_UNHANDLED_REDIRECT, error);
        } else {
            throw new RequestException(STATUS_UNHANDLED_HTTP_CODE, error);
        }
    }
}
