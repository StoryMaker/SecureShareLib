...
public class TransferReceiver extends BroadcastReceiver {
     ...
     * {@link TransferManager#COLUMN_ID}.
     */
    private void openTransfer(Context context, long id) {
    ...
    /**
     * Notify the owner of a running transfer that its notification was clicked.
     */
    private void sendNotificationClickedIntent(Context context, long[] ids) {
        ...
        try {
            if (cursor.moveToFirst()) {
                ...
                Log.w(TAG, "Missing details for transfer " + ids[0]);
                return;
            }
        } finally {
            cursor.close();
